...
(function (root) {
  ...
  var TABLE = [
     ...
     * not already hold, added under the name its author wrote.
     *
     ...
     *
     * Seven of the fifty are not here, because they were the same words in a different order -
     * "Machine Chest Press" and "Chest press machine" are one movement written two ways round, and
     * the library says so already in tidyName. That is the only thing allowed to overrule the sheet.
     *
     * Some of what IS here sits close to an entry above under genuinely different words - "Barbell
     * RDL" beside "Romanian deadlift". That is what taking the sheet at its word costs, and it is
     * the right side to err on: two entries for one movement splits a history, one entry for two
     * movements corrupts it.
     *
     * Generated by tools/gen-programmes.mjs, which prints exactly the rows the sheets still need.
     * Run it again after pasting and it should print nothing.
     */
    'nordic_ham_curl|Nordic Ham Curl|bodyweight|isolation|ha||len',
    ...
    'leg_press_calf_press|Leg Press Calf Press|machine|isolation|ca||len',
    ...
    'chest_supported_dumbbell_row|Chest-Supported Dumbbell Row|dumbbell|horizPull|ub|lt,bi,rd|mid',
    ...
    'lying_reverse_dumbbell_flye|Lying Reverse Dumbbell Flye|dumbbell|isolation|rd|ub|sho',
    ...
    'barbell_rdl|Barbell RDL|barbell|hinge|ha|gl,lb|len',
    'dumbbell_rdl|Dumbbell RDL|dumbbell|hinge|ha|gl,lb|len',
    ...
    'dumbbell_hammer_curl|Dumbbell Hammer Curl|dumbbell|isolation|bi,fa||mid',
    ...
    'dumbbell_wrist_curl|Dumbbell Wrist Curl|dumbbell|isolation|fa||len',
    ...
    'dumbbell_flye|Dumbbell Flye|dumbbell|isolation|ch||len',
    'cable_flye|Cable Flye|cable|isolation|ch||mid',
  ...
  function blocksFromGrid(rows, opts) {
    ...
    function sheetName(name) {
      ...
      return s.split(' ').map(function (w) {
        ...
        if (!full || full.indexOf(' ') !== -1) return w;
    ...
    function resolve(name) {
      ...
      // The same words in a different order are the same movement. "Machine Chest Press" and "Chest
      // press machine" are one exercise written two ways round, and the library has an opinion about
      // this already - tidyName moves a trailing piece of kit to the front for exactly this reason.
      // This is the ONLY thing allowed to override the sheet's own name, and it is not a conversion:
      // no word is added, dropped or changed, so nothing about the movement can differ.
      var pool = all(custom);
      for (var i = 0; i < pool.length; i++) if (sameMovement(pool[i].name, clean)) return pool[i].id;
