...
/* Does this pointer still name a session that is GOING?
 *
 * The pointer is written when the player opens and deleted when the session is signed off, and for a
 * while those were the only two events that mattered. They are not: Finish stamps `endedAt` on the
 * log and THEN shows the buddy's send-off, and the send-off is the last thing a lot of people see
 * before they put the phone down. Kill the app there - or let a phone reload the PWA overnight, or
 * let the other device's copy of `open` come back through a sync - and the pointer outlived the
 * session it named. The tab then opened straight into the player, every single time, on a workout
 * that was already finished and already counted: a live session you could not get out of, because
 * the one control that ends one had already been pressed.
 *
 * So the log has the last word, not the pointer. `sessionOpen` owns the rule - today's date and no
 * `endedAt` - and this is the pointer-shaped spelling of it: named log, or today's log for that
 * session, or today's freeform log. A pointer with no log behind it yet is still live: opening a
 * session and ticking nothing is a session you are in, and there is nothing there to say otherwise. */
function openIsLive(db, o) {
  const t = tdb(db);
  const today = Store.todayISO();
  if (o.logId) {
    const log = t.logs.filter(l => l.id === o.logId)[0];
    return !!log && liveLog(log, today);
  }
  const mine = t.logs.filter(l => (o.sessionId ? l.sessionId === o.sessionId : !l.sessionId));
  const todays = mine.filter(l => (l.dateISO || '') === today || liveLog(l, today));
  return todays.length ? todays.some(l => liveLog(l, today)) : true;
}
 ...
 *
 * Checked against the world before it is trusted: the day has to be today, whatever it points at has
 * to still exist, and the session it names has to still be GOING. A pointer at a block that was
 * deleted underneath it must not put the app into a screen it cannot draw, and a pointer at a
 * session you have already finished must not put you back into it. */
function openRecord(db) {
  const t = tdb(db);
  const o = t.open;
  if (!o || o.atISO !== Store.todayISO()) return null;
  if (o.logId && !t.logs.some(l => l.id === o.logId)) return null;
  if (!openIsLive(db, o)) return null;
  if (o.blockId) {
