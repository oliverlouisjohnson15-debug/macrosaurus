...
const LEGAL_UPDATED = "7 July 2026";
const LEGAL = {
  privacy: {
    ...
    sections: [
      ...
      { h: "Information we collect", p: "Your account details (email, and a password handled securely by our authentication provider); the profile and health inputs you enter (sex, age, height, weight, body fat, activity level and goals); your logs (food, drinks, weigh-ins and check-ins); any photos you choose to submit for AI estimation; records of your AI usage (call counts and cost); and the basic technical data needed to run the app." },
      ...
      { h: "Who we share it with", p: "We use trusted providers to run the app: Supabase (database and sign-in), Vercel (hosting), Anthropic (AI estimates) and Open Food Facts (food lookups). They process data only to provide their service to us. We do not sell your data and we do not use it for advertising." },
      ...
      { h: "Cookies and local storage", p: "We use only essential local storage, to keep you signed in and to work offline. There is no advertising and no third-party tracking." },
...
const CR_ART = {
  egg: ['....KKKK....', '..KKHHHHKK..', '.KHBBBBBBHK.', '.KBBBBSBBBK.', 'KHBBBBBBBSBK', 'KBBBSBBBBBBK', 'KBBBBBBBBSBK', 'KBSBBBBBBBBK', '.KBBBBBSBBK.', '.KBBBBBBBBK.', '..KKBBBBKK..', '....KKKK....'],
  ...
  spino: ['......KKK...', '.....KBBBK..', '..A..KBPBK..', '.AKA.KBBBK..', 'AKAKKBBBBK..', '.KBBBBBBBK..', '.KBBBBBBK...', '.KBBBBBBK...', '.KBKBBKBK...', '.KK.KK.KK...', '............', '............'],
      ...
      <div className="bg-[#0F0F12] w-full max-w-md pixel-box p-5 max-h-[90vh] overflow-y-auto sheet-up" style={{ paddingBottom: 'calc(1.75rem + env(safe-area-inset-bottom))' }} onClick={e => e.stopPropagation()}>
        ...
        {phase === 'select' && <div className="fade-in">
          ...
          <div className="flex items-center justify-between gap-2 p-3 mb-2 pixel-box" style={{ background: 'var(--surface2)', boxShadow: 'none' }}>
            <div className="text-center flex-1 min-w-0"><div className="pixel-box p-2 inline-block" style={{ background: 'var(--surface3)' }}><Sprite art={fighter.art} colors={fighter.colors} px={5} /></div><div className="text-[10px] mt-1.5 font-bold truncate">{fighter.name}</div><StatLine s={fighter.stats} /></div>
...
function RecipeDetail({ recipe, db, update, showToast, onBack, onDelete, onLogRecipe, onSaveMeal }) {
  ...
  // On open, price every ingredient so the recipe always has numbers: free Open Food Facts first,
  // then ONE cheap AI call for whatever OFF couldn't match. Runs once; per-ingredient overrides stay.
  async function autoResolve() {
    const todo = (recipe.ingredients || []).filter(i => !i.macros && i.name && i.grams > 0);
    if (!todo.length) return;
    setBusy('Pricing ingredients...');
    const offRes = await Promise.all(todo.map(i => offResolvePer100(i.name).then(r => ({ id: i.id, r: r })).catch(() => ({ id: i.id, r: null }))));
    const offOk = new Set(offRes.filter(x => x.r).map(x => x.id));
    const remaining = todo.filter(i => !offOk.has(i.id));
    let ai = [];
    if (remaining.length) { try { ai = await aiEstimateIngredientsPer100(remaining.map(i => i.name)); } catch (e) { /* leave for manual */ } }
    patch((r) => {
      offRes.forEach(x => { if (!x.r) return; const ing = r.ingredients.find(y => y.id === x.id); if (!ing || ing.macros) return; ing.resolved = { source: 'off', per100: x.r.per100, product: x.r.product, barcode: x.r.barcode, brand: x.r.brand }; ing.macros = Rcp.macrosFromPer100(x.r.per100, ing.grams); });
      remaining.forEach((i, k) => { const e = ai[k]; if (!e || !e.per100.kcal) return; const ing = r.ingredients.find(y => y.id === i.id); if (!ing || ing.macros) return; ing.resolved = { source: 'ai', per100: e.per100 }; ing.macros = Rcp.macrosFromPer100(e.per100, ing.grams); });
    });
    setBusy('');
  }
  useEffect(() => { if (!autoTried.current && total && resolved < total && recipe.macros_source !== 'stated') { autoTried.current = true; autoResolve(); } }, []);
  ...
  return (<div className="fade-in">
    ...
    {!busy && resolved < total && <div className="flex gap-2 mb-4">
      <Btn kind="accent" className="flex-1" onClick={() => resolveOff(false)} disabled={!!busy}>Look up ingredients (free)</Btn>
      <Btn kind="ghost" onClick={resolveAi} disabled={!!busy}>AI</Btn>
    </div>}
