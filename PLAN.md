...
**CoFID is converted to the label basis at load.** `foods-uk.json` stays faithful to what OHID published, and `Cofid.toLabelBasis` converts it as the app reads it. Two differences made this necessary: CoFID reports available carbohydrate as *monosaccharide equivalents* (100 g of starch yields 110 g of glucose, a disaccharide 105 g), and its energy predates Reg. 1169/2011 so it counts fibre as 0 kcal and carbohydrate at 3.75 kcal/g. Left alone, a CoFID food and a barcoded Open Food Facts product were on different bases in the same diary, so their carbs were not comparable and neither were their calories.

Measured across the table, the conversion drops carbohydrate by **~7%** and moves calories by **~1%** — removing the hydration water and adding fibre's energy nearly cancel — so it corrects a real carb overstatement without disturbing the targets. Sugars carry the disaccharide factor, which over-corrects the free monosaccharides in fruit by a couple of percent; CoFID does not publish the mono/di split needed to do better, and that residual is a fraction of the error removed.
